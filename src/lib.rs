...
//!
//! Currently, this only supports v2.0 of the board, and only uses the IDF
//! version of the ESP crates. If there is interest, older board versions and
//! the alternative bare-metal ESP crates could be added as features.
//!
...
//! Note that the standard ESP-IDF crates (viz. `esp-idf-hal`, `esp-idf-sys`,
//! and `esp-idf-svc`) are re-exported from this crate with their features
//! exposed. A binary crate will still need to include `esp-idf-sys` as a direct
//! dependency.
//!
//! In order to release this crate on [crates.io](https://crates.io/), all dependencies must also be released there.
//! However, there are currently a couple of issues that require dependencies to
//! be patched to `git` repositories:
...
//!
//! I will try to monitor these issues so that this can be released on
//! `crates.io` once they are resolved.
//!
...
/// Initializes the processor, and must be called at the beginning of the `main`
/// function.
pub fn initialize() {
...
/// ```
pub fn i2c_driver<'d, I2C: i2c::I2c + 'd>(
